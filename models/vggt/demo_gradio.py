# coding=utf-8
# Adapted from
# https://github.com/facebookresearch/vggt/blob/evaluation/demo_gradio.py
...
# -------------------------------------------------------------------------
def run_model(target_dir, vggt_model, input_device) -> dict:
    """
    Run the VGGT model on images in the 'target_dir/images' folder and return predictions.
    """
    logging.info(f"Processing images from {target_dir}")
    ...
    logging.info("Computing world points from depth map...")
...
# -------------------------------------------------------------------------
def handle_uploads(inp_video, inp_images):
    """
    ...
    if inp_video is not None:
        if isinstance(inp_video, dict) and "name" in inp_video:
            video_path = inp_video["name"]
        else:
            video_path = inp_video
    ...
    end_time = time.time()
    logging.info(f"Files copied to {target_dir_images}; took {end_time - start_time:.3f} seconds")
...
# -------------------------------------------------------------------------
def update_gallery_on_upload(inp_video, inp_images):
    """
    Whenever user uploads or changes files, immediately handle them
    and show in the gallery. Return (target_dir, image_paths).
    If nothing is uploaded, returns "None" and empty list.
    """
    if not inp_video and not inp_images:
        return None, None, None, None
    target_dir, image_paths = handle_uploads(inp_video, inp_images)
...
def gradio_demo(
    target_dir,
    confidence_thres=3.0,
    input_frame_filter="All",
    mask_black_background=False,
    mask_white_background=False,
    show_camera=True,
    mask_sky_f=False,
    predict_mode="Pointmap Regression",
):
    """
    ...
    logging.info("Running run_model...")
    with torch.no_grad():
    ...
    if input_frame_filter is None:
        input_frame_filter = "All"

    input_frame_filter_string = input_frame_filter.replace('.', '_').replace(':', '').replace(' ', '_')
    # Build a GLB file name
    glbfile = os.path.join(
        target_dir,
        f"glbscene_{confidence_thres}_{input_frame_filter_string}_maskb{mask_black_background}" + 
        f"_maskw{mask_white_background}_cam{show_camera}_sky{mask_sky_f}_pred{predict_mode.replace(' ', '_')}.glb",
    )

    # Convert predictions to GLB
    glbscene = predictions_to_glb(
        predictions,
        conf_thres=confidence_thres,
        filter_by_frames=input_frame_filter,
    ...
    end_time = time.time()
    logging.info(f"Total time: {end_time - start_time:.2f} seconds (including IO)")
    log_msg = f"Reconstruction Success ({len(all_files)} frames). Waiting for visualization."

    return glbfile, log_msg, gr.Dropdown(choices=frame_filter_choices, value=input_frame_filter, interactive=True)
...
def update_visualization(
    target_dir, confidence_thres, input_frame_filter, mask_black_background, 
    mask_white_background, show_camera, mask_sky_f, predict_mode, is_example_f
):
    """
...
# Example images
# -------------------------------------------------------------------------
COLOSSEUUM_VIDEO = "examples/videos/Colosseum.mp4"
ROOM_VIDEO = "examples/videos/room.mp4"
KITCHEN_VIDEO = "examples/videos/kitchen.mp4"
FERN_VIDEO = "examples/videos/fern.mp4"
SINGLE_CARTON_VIDEO = "examples/videos/single_cartoon.mp4"
...
with gr.Blocks(
    ...
    # ---------------------- Examples section ----------------------
    examples = [
        [COLOSSEUUM_VIDEO, "22", None, 20.0, False, False, True, False, "Depthmap and Camera Branch", "True"],
        [PYRAMID_VIDEO, "30", None, 35.0, False, False, True, False, "Depthmap and Camera Branch", "True"],
        [SINGLE_CARTON_VIDEO, "1", None, 15.0, False, False, True, False, "Depthmap and Camera Branch", "True"],
    ...
    def example_pipeline(
        ...
        frame_filter_type = "All"
        glbfile, log_msg, dropdown = gradio_demo(
            target_dir, confidence_thres, frame_filter_type, mask_black_background, \
                mask_white_background, show_camera, mask_sky_f, predict_mode
        )
