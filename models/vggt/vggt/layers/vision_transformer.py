...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None):
        ...
        # The data operated with x must remain consistent with x to avoid being promoted to a higher precision.
        x = torch.cat((self.cls_token.to(x.dtype).expand(x.shape[0], -1, -1), x), dim=1)
        x = x + self.interpolate_pos_encoding(x, w, h)

        if self.register_tokens is not None:
            # The data operated with x must remain consistent with x to avoid being promoted to a higher precision.
            x = torch.cat((x[:, :1], self.register_tokens.to(x.dtype).expand(x.shape[0], -1, -1), x[:, 1:]), dim=1)
