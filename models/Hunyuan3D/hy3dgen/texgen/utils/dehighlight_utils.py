...
class Light_Shadow_Remover():
    ...
    def recorrect_rgb(self, src_image, target_image, alpha_channel, scale=0.95):
        ...
        for i in range(3):
            src_mean = torch.mean(src_flat[:, i].to(torch.float16))
            src_stddev = torch.std(src_flat[:, i].to(torch.float16))

            target_mean = torch.mean(target_flat[:, i].to(torch.float16))
            target_stddev = torch.std(target_flat[:, i].to(torch.float16))
    ...
    def __call__(self, image):
        ...
        if image.mode == 'RGBA':
            ...
            image_array[alpha_channel == 0, :rgb_channel] = 255
        ...
        image = image[:, :, :rgb_channel] * image[:, :, rgb_channel:] + \
            torch.ones_like(image[:, :, :rgb_channel]) * (1.0 - image[:, :, rgb_channel:])
